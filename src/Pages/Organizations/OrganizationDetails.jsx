...
import OrganizationInfo from "../../Components/OrganizationUi/OrganizationInfo";
import CourseList from "../../Components/OrganizationUi/CourseList";

function OrganizationDetails() {
    const loc = useLocation();
    const pathName = loc.pathname.split("/")[2];
    const { isFetchingOrganization, organizationData } = fetchAllOrganization(pathName);
    const instructorData = organizationData?.data;
  
    const [ Blacklisting, setBlacklisting ] = useState(false)
    
    const handleBlacklistOrganization = async () => {
      try {
        setBlacklisting(true)
        const res = await blackListOrganization({ id: pathName })
        if(res.success){
        ...
        <div className="section ml-auto">
          ...
          <div className="w-[97%] m-auto mt-4 tablet:w-full flex flex-col gap-[30px]">
            ...
            <div className="card flex flex-row justify-between">
              ...
              <div
                onClick={handleBlacklistOrganization}
            ...
            {/**BOTTOM CARD */}
            {
              isFetchingOrganization ? (
                ...
                <>
                  {
                    cardState === 'stinfo' && (
                      <OrganizationInfo data={instructorData} />
                    )
                  }
                  {
                    cardState === 'ordlist' && (
