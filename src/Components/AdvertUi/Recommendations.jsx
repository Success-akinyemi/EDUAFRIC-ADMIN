...
function Banner({ data, loading, timeDate, setTimeDate }) {
  ...
  const filteredData = recommendationData.filter(
    (recommendation) =>
      ...
      recommendation?.destination?.toLowerCase().includes(searchTerm?.toLowerCase()) ||
      recommendation?.startDate?.toLowerCase().includes(searchTerm?.toLowerCase())
  );
  ...
  const currentRecommedndation = filteredData.slice(
  ...
  const handleNewBanner = () => {
    navigate('/new-advert-recomendation/noid')
  }
  return (
      ...
      <div className="flex w-full items-center justify-between">
        <div className="flex items-center gap-[50px]">
          <h3 className="text-lg font-semibold text-[#121212]">
            {filteredData.length} Recommendation
          </h3>
      ...
      <div>
        {loading ? (
          ...
          <table className="min-w-full bg-white">
            ...
            <tbody>
              {currentRecommedndation.map((recommendation) => {
                ...
                return (
                  <tr key={recommendation?._id}>
                    <td className="px-6 py-4 text-[14px] text-[#121212] font-normal">
                      <img alt="" src={recommendation?.image} className="w-[60px]" />
                    </td>
                    <td className="px-6 py-4 text-[14px] text-[#121212] font-normal">
                    ...
                    <td className="px-6 py-4 text-[14px] font-normal text-[#13693B]">
                      <div className="flex flex-col">
                        <p className="text-sm font-normal text-[#121212]">{recommendation?.name}</p>
                        <p className="text-sm font-normal text-[#717171]">{recommendation?.type}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-[14px] text-[#121212] font-normal">
                      {recommendation?.organizationUrl}
                    </td>
  ...
  className={`py-[5px] px-[10px] rounded-[100px] ${
    new Date(recommendation?.startDate) > new Date()
      ? "bg-[#D8E0E5] text-[#585858]" // Pending
      : new Date(recommendation?.endDate) > new Date()
  ...
  {new Date(recommendation?.startDate) > new Date()
    ...
    : "Inactive"}
                        ...
                        <div className="absolute z-50 top-8 flex flex-col gap-3 bg-white border-[1px] border-gray-200 shadow-lg rounded-[8px] p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 w-[170px]">
                          <Link
                            to={`/advert-recomendation-info/${recommendation._id}`}
                            className="flex items-center gap-3 text-sm text-[#585858]"
                          >
                            View
                          </Link>
                          <Link
                            to={`/new-advert-recomendation/${recommendation._id}`}
                            className="flex items-center gap-3 text-sm text-[#585858]"
                          >
                            Edit
                          </Link>
                          <button
                            ...
                            onClick={() => handleBlacklsitStudent(recommendation._id)}
                          >
                            { recommendation?.blocked ? 'Account BLocked' : blacklisting ? 'Blacklisting...' : 'Blacklist'}
                          </button>
                        </div>
                      </div>
                    </td>
