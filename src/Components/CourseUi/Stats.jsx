...
import { fetchCourseStats } from "../../Helpers/fetch.api";


function Stats({ timeDate, setTimeDate }) {
  let data
  if(timeDate){
    const { isFetching, statsData } = fetchCourseStats(timeDate?.value)
    data = statsData
  }
  //console.log('course stats Data', data?.data)
  const instructorStatsData = data?.data

  return (
    <div className="flex items-center justify-center gap-6 bg-white">
      {
        instructorStatsData?.map((item, idx) => (
