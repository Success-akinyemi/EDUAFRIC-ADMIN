...
import { fetchOrdersStats } from "../../Helpers/fetch.api";


function Stats({ timeDate, setTimeDate }) {
  let data
  if(timeDate){
    const { isFetching, statsData } = fetchOrdersStats(timeDate?.value)
    data = statsData
  }
  //console.log('stats Data', data?.data)
  const ordersStatsData = data?.data

  return (
    <div className="flex items-center justify-center gap-6 bg-white">
      {
        ordersStatsData?.map((item, idx) => (
